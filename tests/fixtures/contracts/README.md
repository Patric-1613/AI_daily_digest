...
- **`raw_candidates.json` (Pre-Ingestion Raw Inputs)**: Models candidate entries as they arrive from raw feeds *before* ingestion normalization:
  ...
  2. **Changed-URL candidate pair**: Raw entries modeling URL drift via trailing-slash variations on the same scheme (`https://anthropic.example.org/news/claude-3-5-haiku-v1/` vs `https://anthropic.example.org/news/claude-3-5-haiku-v1`) that `canonicalize_url()` and `dedupe_key()` deterministically normalize to an identical canonical URL and identical `dedupe_key`.
