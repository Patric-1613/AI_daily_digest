...
The repository's `render.yaml` declares the three resources needed for the database-backed MVP:
one Free web service, one Free static site, and one Free PostgreSQL database. Cron and Resend are
...
| PostgreSQL | Free Render Postgres declared in `render.yaml`; Alembic owns schema changes | `DATABASE_URL` is injected with a `fromDatabase` reference and normalized to the async psycopg scheme at the typed configuration boundary | Person A owns persistence behavior; Person C owns deployment wiring. |
...
only explicitly Free resources, injects the database by reference, and commits no secret value.
...
2. In Render, create a Blueprint from the GitHub repository and select `render.yaml`. Confirm that
   the plan shows exactly one Free web service, one Free static site, and one Free PostgreSQL
   database; cancel if it proposes a cron job, disk, paid plan, or payment-backed overage.
